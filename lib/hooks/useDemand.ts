...
import { COMPONENT_ITEMS, IngredientName, RecipeItem } from "../items/types";
...
export function useDemand(): DemandItem[] {
  ...
  return useMemo(() => {
    ...
    return Object.entries(fullCost).reduce(
      (acc: DemandItem[], [name, dailyDemand]) => {
...
function mergeIngredientCost(
  a: IngredientCost,
  b: IngredientCost,
): IngredientCost {
  const allEntries = [...Object.entries(a), ...Object.entries(b)] as [
    IngredientName,
    number,
  ][];
...
function getRecipeItemCost(item: RecipeItem): IngredientCost {
  ...
  const recipe =
    COMPONENT_ITEMS.find((c) => c.name === item.name)?.recipe || [];
